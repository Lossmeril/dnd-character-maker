...
const CardNumberInput = ({value, onChange}: CardNumberInputProps) => {
    ...
    return (
        <HStack maxW='320px'>
            <Button {...inc} onClick={() => onChange(value + 1)}>+</Button>
            <Input {...input} />
            <Button {...dec} onClick={() => onChange(value - 1)}>-</Button>
        </HStack>
    )
}
                ...
                class_.abilities.map(FindAbilityDetail).map((ability, i) =>
                    (
                        <Box key={i} mt={4} ml={2}>
                            ...
                            <Text fontSize="smaller" fontStyle="italic">{ability.description}</Text>
                        </Box>
                    )
                )
            }

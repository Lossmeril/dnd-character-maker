...
export const calculateResultStatValue = (
  { race, stats }: Character,
  statId: StatId
): number => {
  const baseValue = stats[statId];
  const modifiers =
    _(FindRaceDetail(race).modifiers)
...
export const getAvailableSpecialAbilities = ({
  classes,
}: Character): SpecialAbilityId[] =>
  _(classes)
    .entries()
    .filter(([_, value]) => value > 0)
    .map(([classId, _]) => classId as unknown as number)
